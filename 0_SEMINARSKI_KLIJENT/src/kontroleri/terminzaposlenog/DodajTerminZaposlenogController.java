 ...
 */
package kontroleri.terminzaposlenog;
    ...
    private void addActionListener() {
        fdtz.dodajTerminZaposlenogAddActionListener(new ActionListener() {
            ...
            private void dodaj(ActionEvent e) {
                ...
                if (fdtz.getjTextFieldDatumRada().getText().isEmpty()) {
                    JOptionPane.showMessageDialog(fdtz, "Morate uneti datum rada.", "Greska", JOptionPane.ERROR_MESSAGE);
                    return;
                }
                ...
                if (datumRada.isBefore(LocalDate.now())) {
                    JOptionPane.showMessageDialog(fdtz, "Datum rada ne moze biti u proslosti.", vreme, JOptionPane.ERROR_MESSAGE);
                    return;
                }

                if (fdtz.getjComboBoxZaposleni().getSelectedIndex() == -1) {
                    JOptionPane.showMessageDialog(fdtz, "Morate odabrati zaposlenog.", "Greska", JOptionPane.ERROR_MESSAGE);
                    return;
                }
                ...
                try {
                    ...
                    String poruka = Komunikacija.getInstance().dodajTerminZaposlenog(zt);

                    if (poruka.length() != 0) {
                        JOptionPane.showMessageDialog(fdtz, poruka, "Greška", JOptionPane.ERROR_MESSAGE);
                        return;
                    } else {
                        JOptionPane.showMessageDialog(fdtz, "Sistem je uspesno dodao zaposlenog!", "Obaveštenje", JOptionPane.INFORMATION_MESSAGE);
                        fdtz.dispose();
                    }

                } catch (Exception ex) {
                    JOptionPane.showMessageDialog(fdtz, "Došlo je do greške u komunikaciji sa serverom.", "GRESKA", JOptionPane.ERROR_MESSAGE);
                }
            }
        ...
        fdtz.azurirajTerminZaposlenogAddActionListener(new ActionListener() {
            ...
            private void izmeni(ActionEvent e) {
                ...
                if (datumRada.isBefore(LocalDate.now())) {
                    JOptionPane.showMessageDialog(fdtz, "Datum rada ne moze biti u proslosti.", "Greska", JOptionPane.ERROR_MESSAGE);
                    return;
                }
                ...
                List<ZapTermin> listaTermina = Komunikacija.getInstance().ucitajTermineZaposlenih();

                for (ZapTermin zz : listaTermina) {
                    if (zz.equals(podaci)) {
                        continue;
                    }

                    if (zz.getIdZaposleni().equals(zt.getIdZaposleni()) && zz.getDatumRada().equals(zt.getDatumRada())) {
                        JOptionPane.showMessageDialog(fdtz, "Sistem ne moze da zapamti termin zaposlenog!", "Greska", JOptionPane.ERROR_MESSAGE);
                        return;
                    }
                }

                ZapTermin stara = (ZapTermin) Cordinator.getInstance().vratiParam("Termin zaposlenog");
                String uslov = "idZaposleni = " + stara.getIdZaposleni().getIdZaposleni() + " AND idTerminDezurstva = " + stara.getIdTerminDezurstva().getIdTerminDezurstva() + " AND datumRada = '" + stara.getDatumRada().toString() + "'";
                try {

                    Komunikacija.getInstance().azurirajTerminZaposlenog(zt, uslov);
                    JOptionPane.showMessageDialog(fdtz, "Sistem je zapamtio termin zaposlenog.", "Uspeh", JOptionPane.INFORMATION_MESSAGE);
                    fdtz.dispose();
                } catch (Exception ex) {
                    JOptionPane.showMessageDialog(fdtz, "Došlo je do greške u komunikaciji sa serverom.", "GRESKA", JOptionPane.ERROR_MESSAGE);
                    return;
